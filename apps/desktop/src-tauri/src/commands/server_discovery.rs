use tauri::State;
use crate::AppState;
use mcpmux_core::{InstalledServer, ServerDefinition, ServerSource, UiConfig, HomeConfig};
...
pub async fn discover_servers(
    ...
    tracing::info!("[discover_servers] Returning {} server definitions", servers.len());
    Ok(servers)
}
...
pub async fn refresh_registry(
    ...
    tracing::info!("[refresh_registry] Refresh complete, {} new servers auto-installed", count);
    Ok(count)
}
...
async fn auto_install_user_servers(
    ...
    for server in servers {
        if let ServerSource::UserSpace { space_id, .. } = &server.source {
            ...
            let existing = state.installed_server_repository
            ...
            if existing.is_none() {
                ...
                let installed = InstalledServer::new(space_id, &server.id)
                    .with_definition(server);
                state.installed_server_repository
                    .install(&installed)
                    .await
...
#[tauri::command]
pub async fn is_registry_offline(
    state: State<'_, AppState>,
) -> Result<bool, String> {
