...
use uuid::Uuid;
use mcpmux_core::{DomainEvent, DiscoveredCapabilities};
...
impl ServerManager {
    ...
    pub async fn get_status(&self, key: &ServerKey) -> Option<(ConnectionStatus, u64, bool, Option<String>)> {
        if let Some(entry) = self.states.get(key) {
            let state = entry.read().await;
            Some((state.status, state.flow_id, state.has_connected_before, state.error.clone()))
        } else {
            None
        }
    }
    ...
    pub async fn get_all_statuses(&self, space_id: Uuid) -> HashMap<String, (ConnectionStatus, u64, bool, Option<String>)> {
        ...
        for entry in self.states.iter() {
            if entry.key().space_id == space_id {
                ...
                result.insert(
                    ...
                    (state.status, state.flow_id, state.has_connected_before, state.error.clone()),
                );
            }
        }
        result
    ...
    fn emit(&self, event: DomainEvent) {
        ...
        if let DomainEvent::ServerStatusChanged { ref server_id, ref status, .. } = event {
    ...
    fn get_or_create_state(&self, key: ServerKey) -> dashmap::mapref::one::Ref<'_, ServerKey, RwLock<ServerState>> {
        let key_clone = key.clone();
        self.states.entry(key).or_insert_with(|| RwLock::new(ServerState::default()));
    ...
    pub async fn enable_server(&self, key: ServerKey) -> Result<(), String> {
        ...
        if matches!(state.status, ConnectionStatus::Connecting | ConnectionStatus::Connected) {
            return Ok(());
        }
        ...
        let _ = self.prefix_cache.assign_prefix_for_server(&space_id_str, &key.server_id).await;
    ...
    pub async fn disable_server(&self, key: &ServerKey) -> Result<(), String> {
        ...
        let had_tools = state.features.as_ref().map(|f| !f.tools.is_empty()).unwrap_or(false);
        let had_prompts = state.features.as_ref().map(|f| !f.prompts.is_empty()).unwrap_or(false);
        let had_resources = state.features.as_ref().map(|f| !f.resources.is_empty()).unwrap_or(false);
        ...
        self.prefix_cache.release_prefix_runtime(&space_id_str, &server_id).await;
        
        // Clean up connection resources if was connected
        if was_connected {
            // Clear tokens and mark features unavailable
            if let Err(e) = self.connection_service
    ...
    #[allow(dead_code)]
    async fn on_connect_complete(&self, key: &ServerKey, flow_id: u64, result: Result<ConnectResult, String>) {
        let entry = match self.states.get(key) {
            Some(e) => e,
            None => return,
        };
    ...
    pub async fn on_auth_callback(&self, key: &ServerKey, flow_id: u64, code: &str) -> Result<(), String> {
        let entry = match self.states.get(key) {
            Some(e) => e,
            None => return Err("Server not found".to_string()),
        };
    ...
    pub async fn startup_refresh(&self, enabled_servers: Vec<(Uuid, String)>) {
        info!(count = enabled_servers.len(), "[RefreshService] Starting refresh for all enabled servers");
    ...
    async fn refresh_single_server(&self, key: &ServerKey) {
        ...
        match state.status {
            ConnectionStatus::Connecting | ConnectionStatus::Authenticating | ConnectionStatus::Refreshing => {
    ...
    async fn on_refresh_complete(&self, key: &ServerKey, result: Result<CachedFeatures, String>) {
        let Some(entry) = self.states.get(key) else { return };
        let mut state = entry.write().await;
        ...
        match result {
            ...
            Err(e) => {
                ...
                if e.contains("auth") || e.contains("token") || e.contains("401") || e.contains("unauthorized") {
                    state.status = ConnectionStatus::AuthRequired;
                    state.error = Some(e.clone());
    ...
    pub fn start_periodic_refresh(self: Arc<Self>) -> JoinHandle<()> {
        tokio::spawn(async move {
            ...
            loop {
                ...
                debug!(count = connected_keys.len(), "[RefreshService] Periodic refresh starting");
...
fn compute_feature_diff(old: &CachedFeatures, new: &CachedFeatures) -> (Vec<String>, Vec<String>) {
    ...
    let old_names: HashSet<&str> = old.tools.iter().map(|t| t.feature_name.as_str())
        .chain(old.prompts.iter().map(|p| p.feature_name.as_str()))
        .chain(old.resources.iter().map(|r| r.feature_name.as_str()))
        .collect();
    
    let new_names: HashSet<&str> = new.tools.iter().map(|t| t.feature_name.as_str())
        .chain(new.prompts.iter().map(|p| p.feature_name.as_str()))
        .chain(new.resources.iter().map(|r| r.feature_name.as_str()))
        .collect();
    
    let added: Vec<String> = new_names.difference(&old_names).map(|s: &&str| s.to_string()).collect();
