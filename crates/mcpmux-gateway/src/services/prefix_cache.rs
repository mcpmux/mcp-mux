...
impl SpacePrefixCache {
    ...
    fn assign(&mut self, server_id: String, prefix: String) {
        self.server_to_prefix
            .insert(server_id.clone(), prefix.clone());
...
impl PrefixCacheService {
    ...
    pub async fn resolve_prefixes_on_startup(&self, space_id: &str) -> anyhow::Result<()> {
        ...
        info!(
            "[PrefixCache] Resolving prefixes for space {} (startup)",
            space_id
        );
    ...
    pub async fn resolve_qualified_name(
        &self,
        space_id: &str,
        qualified_name: &str,
        ...
        let server_id = self
            .get_server_for_prefix(space_id, prefix)
            .await
...
mod tests {
    ...
    async fn test_assign_and_lookup() {
        ...
        // Assign with alias
        let prefix = service
            .assign_prefix_runtime(space_id, server_id, Some("cf"))
            .await;
        assert_eq!(prefix, "cf");
    ...
    async fn test_conflict_uses_fallback() {
        ...
        let prefix1 = service
            .assign_prefix_runtime(space_id, "server-a", Some("api"))
            .await;
    ...
    async fn test_release_prefix() {
        ...
        // Verify assigned
        assert_eq!(
            service.get_prefix_for_server(space_id, server_id).await,
            "cf"
        );
    ...
    async fn test_resolve_qualified_name() {
        ...
        service
            .assign_prefix_runtime(space_id, server_id, Some("azuremcp"))
            .await;

        // Resolve "azuremcp_group_list" - underscore separates prefix from feature
        let result = service
            .resolve_qualified_name(space_id, "azuremcp_group_list")
            .await;
        assert_eq!(
            result,
            Some((server_id.to_string(), "group_list".to_string()))
        );

        // Resolve "azuremcp_documentation"
        let result = service
            .resolve_qualified_name(space_id, "azuremcp_documentation")
            .await;
        assert_eq!(
            result,
            Some((server_id.to_string(), "documentation".to_string()))
        );
    }
    ...
    async fn test_resolve_simple_prefix() {
        ...
        service
            .assign_prefix_runtime(space_id, server_id, Some("gh"))
            .await;
