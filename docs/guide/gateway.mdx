...
## Web admin mode (remote UI)

For operators who need the full McpMux UI from another device (phone, laptop) without screen sharing, enable **Web admin mode** in **Settings → Gateway**:

- **Admin server** listens on `127.0.0.1:45819` by default (configurable port, never bind to `0.0.0.0`)
- Serves the production React build at `/` and REST at `/api/v1/*`
- **Cloudflare Access** (recommended): expose a dedicated admin hostname (e.g. `mux.example.com`) via `cloudflared` to `localhost:45819` and enable **Trust Cloudflare Access JWT** so only authenticated tunnel traffic reaches the app
- **Local dev**: disable CF JWT trust to hit `http://127.0.0.1:45819` directly
- **Health checks**: when CF Access trust is enabled, `/api/v1/health` also requires a valid `CF-Access-Jwt-Assertion` header. Cloudflare Tunnel origin probes do not send that header — use an external monitor or keep trust disabled on a dev-only admin instance rather than relying on tunnel health checks against `:45819`.
- **Missing SPA build**: if `pnpm build:web:admin` has not been run, visiting `/` returns a 503 HTML page with build instructions instead of a blank 404.

Build the SPA for admin serving from the repo root:

```bash
pnpm build:web:admin
```

The Tauri app resolves `apps/desktop/dist` when the admin server starts. MCP clients should continue using `http://localhost:45818/mcp` (or a separate MCP tunnel hostname such as `mcp.example.com`) — do not mix admin UI traffic onto the MCP gateway port.
...
- [Remote Access](/docs/remote-access/) — optional HTTPS MCP and admin via tunnel + Access
