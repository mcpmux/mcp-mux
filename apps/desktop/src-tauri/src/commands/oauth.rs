...
pub fn handle_deep_link<R: tauri::Runtime>(app: &tauri::AppHandle<R>, url: &str) {
    ...
    if !branding::is_deep_link(url) {
        warn!("[DeepLink] Invalid scheme, expected {}://", branding::DEEP_LINK_SCHEME);
        return;
    }
...
fn handle_authorize_deep_link<R: tauri::Runtime>(app: &tauri::AppHandle<R>, url: &Url) {
    ...
    info!("[DeepLink] Authorization request received: request_id='{}'", request_id);
...
impl ConsentError {
    fn not_found(request_id: &str) -> Self {
        Self {
            ...
            message: format!("Authorization request '{}' not found or expired", request_id),
        }
    }
    ...
    fn already_processed(request_id: &str) -> Self {
        Self {
            ...
            message: format!("Authorization request '{}' has already been processed", request_id),
        }
    }
...
pub async fn get_pending_consent(
    ...
    info!("[OAuth] Fetching pending consent: request_id='{}'", request_id);
    
    let app_state = gateway_state.read().await;
    
    // Get gateway state
    let gw_state = app_state.gateway_state.as_ref()
    ...
    let details = ConsentRequestDetails {
        ...
        client_name: auth.client_name.clone().unwrap_or_else(|| auth.client_id.clone()),
    ...
    info!("[OAuth] Consent details validated: client='{}' scopes='{}'", 
...
pub async fn approve_oauth_consent(
    ...
    let pending = {
        let state = gw_state.read().await;
        state.pending_authorizations.get(&request.request_id).cloned()
    };
            ...
            if let Ok(Some(mut client)) = repo.get_client(&pending.client_id).await {
                ...
                if let Some(alias) = &request.client_alias {
                    if !alias.is_empty() {
                        ...
                        changed = true;
                        info!("[OAuth] Set client alias '{}' for: {}", alias, pending.client_id);
                    }
                }
                ...
                if let Some(mode) = &request.connection_mode {
                    ...
                    changed = true;
                    info!("[OAuth] Set connection mode '{}' for: {}", mode, pending.client_id);
                }
                ...
                if let Some(space_id) = &request.locked_space_id {
                    ...
                    changed = true;
                    info!("[OAuth] Locked to space '{}' for: {}", space_id, pending.client_id);
...
pub async fn get_oauth_clients(
    ...
    let all_clients = repo.list_clients()
        .await
        .map_err(|e| format!("Failed to fetch clients: {}", e))?;
    
    let clients: Vec<_> = all_clients.into_iter().filter(|c| c.approved).collect();

    info!("[OAuth] Fetched {} approved clients from repository", clients.len());
    
    // Map to response format
    let client_infos: Vec<OAuthClientInfo> = clients.into_iter().map(|client| {
        OAuthClientInfo {
...
pub async fn update_oauth_client(
    ...
    // Get updated client
    let updated_client = repo.get_client(&client_id)
        .await
...
pub async fn get_oauth_client_grants(
    ...
    let mut grants = repo.get_grants_for_space(&client_id, &space_id)
        .await
        .map_err(|e| format!("Failed to get grants: {}", e))?;
    
    // Add default feature set (layered resolution - same as MCP handlers)
    if let Ok(Some(default_fs)) = app_state.feature_set_repository.get_default_for_space(&space_id).await {
...
pub async fn grant_oauth_client_feature_set(
    ...
    info!("[OAuth] Gateway state exists: {}", app_state.gateway_state.is_some());
    info!("[OAuth] Grant service exists: {}", app_state.grant_service.is_some());
    ...
    let Some(ref grant_service) = app_state.grant_service else {
        error!("[OAuth] Grant service is None! Gateway running={}, gateway_state={}", 
            app_state.running, app_state.gateway_state.is_some());
        return Err("Gateway not running".to_string());
    };
    ...
    // Notify UI
    if let Err(e) = app_handle.emit("oauth-client-changed", serde_json::json!({
...
pub async fn get_oauth_client_resolved_features(
    ...
    let mut feature_set_ids = repo.get_grants_for_space(&client_id, &space_id)
        .await
        .map_err(|e| format!("Failed to get grants: {}", e))?;
    ...
    info!("[OAuth] Client {} has {} effective grants in space {}", client_id, feature_set_ids.len(), space_id);
    ...
    let tools_response: Vec<_> = tools.iter().map(|f| serde_json::json!({
        "name": f.feature_name,
        "description": f.description,
        "server_id": f.server_id,
    })).collect();
    
    let prompts_response: Vec<_> = prompts.iter().map(|f| serde_json::json!({
        "name": f.feature_name,
        "description": f.description,
        "server_id": f.server_id,
    })).collect();
    
    let resources_response: Vec<_> = resources.iter().map(|f| serde_json::json!({
        "name": f.feature_name,
        "description": f.description,
        "server_id": f.server_id,
    })).collect();
...
pub async fn delete_oauth_client(
    ...
    // Emit domain event
    state.emit_domain_event(mcpmux_core::DomainEvent::ClientDeleted {
        client_id,
    });
        ...
        match client.get(&url).send().await {
            Ok(response) => {
                ...
                if status.is_success() || status.is_redirection() {
                    info!("[OAuth] Callback delivered successfully (status: {})", status);
                } else {
                    // Some clients return non-2xx but still process the code
                    warn!("[OAuth] Callback returned status {}, but code was delivered", status);
                }
                Ok(())
            }
            Err(e) => {
