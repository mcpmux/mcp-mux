...
pub async fn get_server_statuses(
    ...
    let manager = manager_state.manager.as_ref()
        .ok_or("ServerManager not initialized")?;
    
    let statuses = manager.get_all_statuses(space_uuid).await;
    
    Ok(statuses.into_iter().map(|(server_id, (status, flow_id, has_connected, msg))| {
        (server_id.clone(), ServerStatusResponse {
            server_id,
            status,
            flow_id,
...
pub async fn enable_server_v2(
    ...
    let installed = app_state.installed_server_repository
    ...
    app_state.installed_server_repository
    ...
    let manager = manager_state.manager.as_ref()
        .ok_or("ServerManager not initialized")?
        .clone();
    let pool_service = manager_state.pool_service.as_ref()
    ...
    match result {
        ...
        ConnectionResult::OAuthRequired { .. } => {
            ...
            if let Some(ref feature_service) = gateway_state.read().await.feature_service {
                if let Err(e) = feature_service.mark_unavailable(&space_id, &server_id).await {
...
pub async fn disable_server_v2(
    ...
    pool_service.oauth_manager().cancel_flow_for_space(space_uuid, &server_id);
    ...
    info!("[ServerManager] Server {} disabled (features unavailable, tokens preserved)", server_id);
    Ok(())
}
...
pub async fn start_auth_v2(
    ...
    if manager.is_status(&key, ConnectionStatus::Authenticating).await {
        // Check debounce
...
pub async fn cancel_auth_v2(
        ...
        .await
    {
        if let Err(e) = app_state
            ...
            .await
        {
            warn!("[ServerManager] Failed to clear oauth_connected for {}: {}", server_id, e);
        } else {
            info!("[ServerManager] Cleared oauth_connected for cancelled OAuth flow: {}", server_id);
        }
    }
    
    // Reset to AuthRequired state
    manager.set_auth_required(&key, Some("Authentication cancelled".to_string())).await;
...
pub async fn retry_connection(
    ...
    // First, remove the existing instance to force fresh connection with new config
    {
        ...
        if let Some(pool_service) = manager_state.pool_service.as_ref() {
            pool_service.remove_instance(space_uuid, &server_id);
            info!("[ServerManager] Removed existing instance for retry: {}", server_id);
        }
    }
...
pub async fn logout_server(
    ...
    if let Err(e) = app_state.credential_repository.delete(&space_uuid, &server_id).await {
        warn!("[ServerManager] Failed to delete credentials for {}: {}", server_id, e);
    } else {
    ...
    manager.set_auth_required(&key, Some("Logged out - please reconnect".to_string())).await;
    ...
    info!("[ServerManager] Server {} logged out (tokens cleared, features unavailable)", server_id);
...
pub async fn disconnect_server_v2(
    ...
    info!("[ServerManager] Server {} disconnected (features unavailable, credentials preserved)", server_id);
